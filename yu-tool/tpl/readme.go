...
# 1.快速创建代码仓库请使用下方命令
` + "```" + `
yu-tool newService github.com/yunixiangfeng/gopaas/{{.Alias}}

注意：
1.这里github.com/yunixiangfeng/gopaas/{{.Alias}} 仓库名字需要和 go mod 名称一致
` + "```" + `
...
# 1.快速创建代码仓库请使用下方命令
` + "```" + `
yu-tool createApi github.com/yunixiangfeng/gopaas/{{.Alias}}
