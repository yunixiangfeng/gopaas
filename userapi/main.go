...
import (
    ...
    go_micro_service_user "github.com/yunixiangfeng/gopaas/user/proto/user"
	...
	"net"
	"net/http"
	"github.com/yunixiangfeng/gopaas/userApi/handler"
	hystrix2 "github.com/yunixiangfeng/gopaas/userApi/plugin/hystrix"
	"strconv"
...
func main() {
	...
	//2.添加链路追踪
	t,io,err := common.NewTracer("go.micro.api.userApi",tracerHost+":"+strconv.Itoa(tracerPort))
	if err !=nil {
		common.Error(err)
	}
	defer io.Close()
   ...
   // 注册控制器
	if err := userApi.RegisterUserApiHandler(service.Server(), &handler.UserApi { UserService:userService});err !=nil {
		common.Error(err)
	}
